...
# Change email
@method_decorator(login_required, name='dispatch')
class ProfileVerificationView(FormView):
	...
	def get_form_kwargs(self):
		...
		kwargs['user_hashed_password'] = self.request.user.password
		return kwargs

	def form_valid(self, form):
		self.request.session['access_change_email'] = True
		messages.add_message(self.request, messages.SUCCESS, 'The code has been sent on your new email.')
...
class ChangeEmailConfirmView(EmailConfirmationCode, FormView):
	...
	def dispatch(self, request, *args, **kwargs):
		if 'access_change_email' in self.request.session.keys():
...
class ConfirmNewEmailView(EmailConfirmationCode, FormView):
	...
	def form_valid(self, form):
		...
		return super(ConfirmNewEmailView, self).form_valid(form)
